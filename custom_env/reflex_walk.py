...
class Reflex_WALK_Env(mujoco_env.MujocoEnv, utils.EzPickle):
    ...
    def __init__(self,path):
        ...
        mujoco_env.MujocoEnv.__init__(self, os.path.join(path, "reflex_model.xml"), 4)
    ...
    def grf(self):
        ...
        for i in range(self.sim.data.ncon):
            ...
            if self.sim.model.geom_id2name(contact.geom2)== 'left_toe_geom':
                ...
                left_z_N += c_array[0]
        # print(right_touch,left_touch)
        return right_touch, left_touch, right_x_N, left_x_N, right_z_N, left_z_N

    def joints_angles(self):
        theta = self.sim.data.get_joint_qpos("torso_joint")
        dot_theta = self.sim.data.get_joint_qvel("torso_joint")
        hip = self.sim.data.get_joint_qpos("hip_joint")
        L_hip = self.sim.data.get_joint_qpos("left_hip_joint")
        dot_hip = self.sim.data.get_joint_qvel("hip_joint")
        dot_L_hip = self.sim.data.get_joint_qvel("left_hip_joint")
        knee = self.sim.data.get_joint_qpos("knee_joint")
        L_knee = self.sim.data.get_joint_qpos("left_knee_joint")
        dot_knee = self.sim.data.get_joint_qvel("knee_joint")
        dot_L_knee = self.sim.data.get_joint_qvel("left_knee_joint")
        ankle = self.sim.data.get_joint_qpos("ankle_joint")
        L_ankle = self.sim.data.get_joint_qpos("left_ankle_joint")
        dot_ankle = self.sim.data.get_joint_qvel("ankle_joint")
        dot_L_ankle = self.sim.data.get_joint_qvel("left_ankle_joint")
        return np.array([theta, dot_theta, hip, L_hip, dot_hip, dot_L_hip, knee, L_knee, dot_knee, dot_L_knee, ankle, L_ankle, dot_ankle, dot_L_ankle])

    def joints_pos(self):
        hip_x_pos = self.sim.data.get_site_xpos("s_hip")[0]
        hip_z_pos = self.sim.data.get_site_xpos("s_hip")[2]
        r_knee_x_pos = self.sim.data.get_site_xpos("s_knee")[0]
        r_knee_z_pos = self.sim.data.get_site_xpos("s_knee")[2]
        l_knee_x_pos = self.sim.data.get_site_xpos("s_left_knee")[0]
        l_knee_z_pos = self.sim.data.get_site_xpos("s_left_knee")[2]
        r_ankle_x_pos = self.sim.data.get_site_xpos("s_ankle2")[0]
        r_ankle_z_pos = self.sim.data.get_site_xpos("s_ankle2")[2]
        l_ankle_x_pos = self.sim.data.get_site_xpos("s_left_ankle2")[0]
        l_ankle_z_pos = self.sim.data.get_site_xpos("s_left_ankle2")[2]
        r_toe1_x_pos = self.sim.data.get_site_xpos("s_toe1")[0]
        r_toe1_z_pos = self.sim.data.get_site_xpos("s_toe1")[2]
        l_toe1_x_pos = self.sim.data.get_site_xpos("s_left_toe1")[0]
        l_toe1_z_pos = self.sim.data.get_site_xpos("s_left_toe1")[2]
        r_toe2_x_pos = self.sim.data.get_site_xpos("s_toe2")[0]
        r_toe2_z_pos = self.sim.data.get_site_xpos("s_toe2")[2]
        l_toe2_x_pos = self.sim.data.get_site_xpos("s_left_toe2")[0]
        l_toe2_z_pos = self.sim.data.get_site_xpos("s_left_toe2")[2]
        return np.array([hip_x_pos, hip_z_pos, r_knee_x_pos, r_knee_z_pos, l_knee_x_pos, l_knee_z_pos, r_ankle_x_pos, r_ankle_z_pos, l_ankle_x_pos, l_ankle_z_pos, r_toe1_x_pos, r_toe1_z_pos, l_toe1_x_pos, l_toe1_z_pos, r_toe2_x_pos, r_toe2_z_pos, l_toe2_x_pos, l_toe2_z_pos])