...
import os
import pickle
from reflex_opt.reward import cost, cost_fin
...
def optimize_back_timedelay(env):
    ...
    def objective_function(params, tar_vel):
        ...
        for j in range(5000):
            ...
            score += cost(env,d,tar_vel=tar_vel,num=j)

            if d:
                cot = energy/abs(env.pos())/9.8/env.model_mass()
                score += cost_fin(env,energy,tar_vel, np.average(vel))
    ...
    mean=np.array([
        8.58774846e-03,  1.48843881e-02,  8.21440524e-03,  5.57125124e-03,
        1.35517495e-02,  1.04940035e-02,  2.43993317e-02,  2.80769176e-02,
        6.36804430e-03,  4.56026590e-02,  1.61703306e-02,  7.45188956e-03,
        5.05966380e-03,  4.50428201e-02,  1.19775183e-02,  3.86878397e-03,
        1.38386031e+00,  1.32074027e+00,  1.81155732e+00,  7.40580528e-01,
        6.61635902e-01,  9.93223996e-01,  1.87113203e-02,  2.64757779e-02,
        6.33444990e+00,  4.96438163e+00,  5.92606256e-02,  4.50128789e-03,
        5.37638176e-02,  3.81047348e-01,  3.03689637e+00,  1.97156946e-01,
        4.37646205e-01,  1.37256344e-02,  2.80750146e+00,  3.25924272e+00,
        1.81345025e+00,  3.12314575e-01,  4.53356693e-01,  7.95549959e-04,
        5.89184675e-01,  2.78755921e-02,  4.38919180e-01,  1.57736542e-01,
        -2.28517272e-03, 4.19809169e-01,  2.70722368e-01,  9.25537866e-02,
        5.50564710e-01,  1.33680826e+00,  4.38648586e-01,  2.01587732e-01,
        2.90297107e+00,  1.29846794e+00,  4.97056646e-02,  3.54437005e-02])
    ...
    ***IMPORTANT***
    ...
    if not previous_data:
        ...
        save_data = True
        if save_data:
            forder_name = 'review_back_timedelay_new'
            save_folder = save_path + '/' + forder_name
    ...
    for vel_generation in range(vel_generation_checkpoint,vel_generations):
        ...
        for generation in range(generation_checkpoint,generations):
            ...
            # population_size = 4 + math.floor(3 * math.log(n_dim))  # (eq. 48) n_dim=52 ->12
            for trial in range(optimizer.population_size):
                if vel_generation==0 and generation<3 and trial==0:
                    x = mean0
                    store = False
                    print('given')
                else:
                ...
                if not(vel_generation==0 and generation<3 and trial==0):
